...
# 讀取資料集
# data_length = int(pd.read_csv("./new_train_0608.csv").drop(columns=['conv_id']).shape[0]/5)
# print(data_length)
# train_df = pd.read_csv("./new_train_0608.csv").drop(columns=['conv_id'])
# valid_df = pd.read_csv("./new_valid_0608.csv").drop(columns=['conv_id'])

# train = pd.concat([train_df.iloc[:data_length*3,:],train_df.iloc[data_length*4:,:],valid_df],axis=0)
# valid = train_df.iloc[data_length*3:data_length*4,:]

train = pd.read_csv("./new_train_0608.csv").drop(columns=['conv_id'])
valid = pd.read_csv("./new_valid_0608.csv").drop(columns=['conv_id'])
...
validloader = DataLoader(validset, batch_size=16, collate_fn=create_mini_batch)


NUM_LABELS = 32
hidden_dropout_prob=0.3

# model = BertForSequenceClassification.from_pretrained("bert-large-uncased",num_labels=NUM_LABELS,hidden_dropout_prob=hidden_dropout_prob)
...
model.load_state_dict(torch.load("./checkpoint_6_0.6529.pth"))
...
for epoch in range(EPOCHS):
    ...
    _, valid_fl, valid_loss = get_predictions(model, validloader, compute_acc=True)
    print('[epoch %d] valid loss: %.3f, valid f1: %.3f' %(epoch + 1, valid_loss, valid_fl))
    
    if valid_fl>=0.656:
        torch.save(model.state_dict(), './checkpoint_'+str(round(valid_fl,4))+'.pth')
        break